...
while True:
    ...
    # plength = data[4] + (data[5] << 2)
    # print('Length: ', plength, '[{}]'.format(', '.join(hex(x) for x in data[4:6])))
    # print('Payload:\n==========')
    # print(bytes(data[6:(6 + plength)]).decode('utf8', errors='replace'))
    # print('==========')
    # print('Checksum (CK_A CK_B): [{}]'.format(', '.join(hex(x) for x in data[(6 + plength):(6 + plength + 2)])))
    ...
    # print('\n')
    if data:
        print(bytes(data).decode('utf8', errors='replace'))
        # print('\n')
